...
app=FastAPI(title='G.E.N.C Live')
...
def hp(p): return hashlib.sha256(('genc-live-v1:'+p).encode()).hexdigest()
...
class StudentIn(BaseModel):
    email:str; password:str; first_name:str; last_name:str; birth_date:str; city:str; education:str; school:str=''; department:str=''; field:str; bio:str=''
class CompanyIn(BaseModel):
    password:str; legal_name:str; trade_name:str=''; company_type:str; sector:str; tax_number:str=''; mersis:str=''; city:str; authorized_name:str; official_email:str
class LoginIn(BaseModel): email:str; password:str; role:str

@app.get('/api/health')
def health(): return {'ok':True}
@app.post('/api/student/register')
def sreg(d:StudentIn):
    if len(d.password)<8: raise HTTPException(400,'Şifre en az 8 karakter olmalı.')
    try:
        with db() as c:
            cur=c.execute('INSERT INTO users(email,password,role) VALUES(?,?,?)',(d.email.lower(),hp(d.password),'student')); uid=cur.lastrowid
            ...
            tok=secrets.token_urlsafe(32); c.execute('INSERT INTO sessions VALUES(?,?)',(tok,uid))
        return {'token':tok,'role':'student'}
    except sqlite3.IntegrityError: raise HTTPException(409,'Bu e-posta zaten kayıtlı.')
@app.post('/api/company/register')
def creg(d:CompanyIn):
    if len(d.password)<8: raise HTTPException(400,'Şifre en az 8 karakter olmalı.')
    try:
        with db() as c:
            cur=c.execute('INSERT INTO users(email,password,role) VALUES(?,?,?)',(d.official_email.lower(),hp(d.password),'company')); uid=cur.lastrowid
...
def login(d:LoginIn):
    with db() as c:
        ...
        tok=secrets.token_urlsafe(32); c.execute('INSERT INTO sessions VALUES(?,?)',(tok,u['id']))
    return {'token':tok,'role':d.role}
@app.get('/api/me')
def me(token:str):
    u=auth(token)
    if not u: raise HTTPException(401,'Oturum geçersiz.')
    with db() as c:
        p=c.execute('SELECT * FROM students WHERE user_id=?',(u['id'],)).fetchone() if u['role']=='student' else c.execute('SELECT * FROM companies WHERE user_id=?',(u['id'],)).fetchone()
    return {'user':u,'profile':dict(p) if p else None}

HTML='''<!doctype html><html lang="tr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>G.E.N.C</title><style>
:root{--bg:#071019;--card:#0e1b28;--card2:#112434;--text:#f3f7f7;--muted:#92a6b6;--line:rgba(255,255,255,.09);--a:#68e3c4}*{box-sizing:border-box}body{margin:0;background:radial-gradient(900px 600px at 10% -10%,rgba(104,227,196,.09),transparent 60%),var(--bg);color:var(--text);font-family:Inter,system-ui,sans-serif;min-height:100vh}button,input,select,textarea{font:inherit}.wrap{width:min(1180px,calc(100% - 36px));margin:auto}.brand{font-weight:900;letter-spacing:.18em}.brand b{color:var(--a)}.hero{min-height:100vh;display:flex;flex-direction:column}.head{height:78px;display:flex;align-items:center;justify-content:space-between}.main{flex:1;display:flex;align-items:center}.ey{color:var(--a);font-size:12px;font-weight:900;letter-spacing:.2em;text-transform:uppercase}.hero h1{font-size:clamp(50px,8vw,94px);line-height:.96;letter-spacing:-.06em;margin:18px 0}.hero h1 em{color:var(--a);font-style:normal}.hero p{max-width:650px;color:#b8c7d1;font-size:18px;line-height:1.7}.actions{display:flex;gap:10px;flex-wrap:wrap;margin-top:28px}.btn{border:0;border-radius:12px;padding:12px 16px;background:var(--a);color:#06211a;font-weight:900;cursor:pointer}.btn.alt{background:#14283a;color:var(--text);border:1px solid var(--line)}.values{border-top:1px solid var(--line);min-height:78px;display:flex;align-items:center;gap:12px;color:#c5d1d9}.values b{color:var(--a)}.auth{min-height:100vh;display:grid;grid-template-columns:.9fr 1.1fr}.aside{padding:55px;border-right:1px solid var(--line);display:flex;flex-direction:column;justify-content:space-between}.aside h1{font-size:clamp(38px,5vw,66px);line-height:1.02;letter-spacing:-.05em}.aside p{color:var(--muted);line-height:1.7}.pane{display:flex;align-items:center;justify-content:center;padding:38px}.card{width:min(680px,100%);background:linear-gradient(180deg,var(--card2),var(--card));border:1px solid var(--line);border-radius:20px;padding:28px}.card h2{font-size:32px;letter-spacing:-.04em}.form{display:grid;gap:15px}.row{display:grid;grid-template-columns:1fr 1fr;gap:12px}.field{display:grid;gap:7px}.field label{font-size:13px;color:#c7d2da;font-weight:800}.in{width:100%;background:#08131e;border:1px solid rgba(255,255,255,.12);border-radius:11px;padding:13px 14px;color:var(--text);outline:none}.in:focus{border-color:rgba(104,227,196,.65)}.muted{color:var(--muted)}.dash{padding:40px 0}.top{position:sticky;top:0;background:rgba(7,16,25,.9);backdrop-filter:blur(16px);border-bottom:1px solid var(--line)}.topin{height:68px;display:flex;align-items:center;justify-content:space-between}.grid{display:grid;grid-template-columns:repeat(3,1fr);gap:16px}.box{background:var(--card);border:1px solid var(--line);border-radius:17px;padding:22px}.box small{color:var(--muted);font-weight:800}.box strong{display:block;font-size:28px;margin-top:10px}.notice{padding:15px;border-radius:13px;border:1px solid rgba(104,227,196,.22);background:rgba(104,227,196,.06);color:#bdd7cf}.toast{position:fixed;right:20px;top:20px;background:#102538;border:1px solid rgba(104,227,196,.24);padding:13px 15px;border-radius:12px;display:none}.toast.show{display:block}@media(max-width:850px){.auth{grid-template-columns:1fr}.aside{min-height:300px;border-right:0;border-bottom:1px solid var(--line);padding:30px}.pane{padding:24px 16px}.grid,.row{grid-template-columns:1fr}.head{height:68px}.hero p{font-size:16px}.actions{flex-direction:column}}
</style></head><body><div id="app"></div><div id="toast" class="toast"></div><script>
let token=localStorage.genc_token||'',role=localStorage.genc_role||'';const A=document.getElementById('app');function go(p){location.hash=p}function p(){return location.hash.slice(1)||'/'}function t(m){let x=document.getElementById('toast');x.textContent=m;x.classList.add('show');setTimeout(()=>x.classList.remove('show'),2500)}async function api(u,o={}){o.headers={'Content-Type':'application/json',...(o.headers||{})};let r=await fetch('/api'+u,o),d=await r.json().catch(()=>({}));if(!r.ok)throw Error(d.detail||'İşlem başarısız');return d}function brand(){return '<span class="brand">G.E.N<b>.C</b></span>'}
function home(){return `<main class="hero"><div class="wrap head">${brand()}<div class="actions" style="margin:0"><button class="btn alt" onclick="go('/student/login')">Öğrenci Girişi</button><button class="btn alt" onclick="go('/company/login')">Şirket Girişi</button></div></div><div class="wrap main"><div><div class="ey">Kariyer ve deneyim platformu</div><h1>Deneyimin yoksa,<br><em>burada kazan.</em></h1><p>Gerçek iş senaryolarında kendini geliştir. Yeteneklerini kanıtla. Kariyer profilini oluşturmaya başla.</p><div class="actions"><button class="btn" onclick="go('/student/register')">Öğrenci olarak başla →</button><button class="btn alt" onclick="go('/company/register')">Şirket olarak devam et →</button></div></div></div><div class="wrap values"><span>Gerçek görevler</span><b>•</b><span>Doğrulanabilir beceriler</span><b>•</b><span>Kariyer profili</span></div></main>`}
function shell(kind,inside){let student=kind==='student';return `<main class="auth"><aside class="aside"><div>${brand()}<h1>${student?'Kariyerini CV’den önce işinle göster.':'CV yığınını değil, gerçek performansı değerlendir.'}</h1><p>${student?'Ad-soyad, eğitim ve kariyer bilgilerinle profesyonel profilini oluştur.':'Kurumsal bilgilerin doğrulandıktan sonra gerçek görev yayınla ve adayları iş çıktıları üzerinden değerlendir.'}</p></div><p>G.E.N.C · Doğrulanabilir deneyim altyapısı</p></aside><section class="pane">${inside}</section></main>`}
...
async function student(e){e.preventDefault();try{let d=await api('/student/register',{method:'POST',body:JSON.stringify({email:email.value,password:pw.value,first_name:fn.value,last_name:ln.value,birth_date:bd.value,city:city.value,education:edu.value,school:school.value,department:dep.value,field:field.value,bio:bio.value})});token=d.token;role=d.role;localStorage.genc_token=token;localStorage.genc_role=role;go('/dashboard')}catch(e){t(e.message)}}async function company(e){e.preventDefault();try{let d=await api('/company/register',{method:'POST',body:JSON.stringify({password:cpw.value,legal_name:legal.value,trade_name:trade.value,company_type:ctype.value,sector:sector.value,tax_number:tax.value,mersis:mersis.value,city:ccity.value,authorized_name:authname.value,official_email:cemail.value})});token=d.token;role=d.role;localStorage.genc_token=token;localStorage.genc_role=role;go('/dashboard')}catch(e){t(e.message)}}async function log(e,k){e.preventDefault();try{let d=await api('/login',{method:'POST',body:JSON.stringify({email:le.value,password:lp.value,role:k})});token=d.token;role=d.role;localStorage.genc_token=token;localStorage.genc_role=role;go('/dashboard')}catch(e){t(e.message)}}
async function dashboard(){if(!token){go('/');return''}try{let d=await api('/me?token='+encodeURIComponent(token));let q=d.profile;if(role==='student')return `<header class="top"><div class="wrap topin">${brand()}<button class="btn alt" onclick="out()">Çıkış</button></div></header><main class="dash"><div class="wrap"><div class="ey">Öğrenci Paneli</div><h1>${q.first_name} ${q.last_name}</h1><p class="muted">${q.city} · ${q.education} · ${q.field}</p><div class="grid"><div class="box"><small>G.E.N.C Score</small><strong style="font-size:20px">Henüz oluşmadı</strong></div><div class="box"><small>Doğrulanmış deneyim</small><strong>0</strong></div><div class="box"><small>Profil</small><strong>${q.school?'Güçlü':'Geliştir'}</strong></div></div><div style="margin-top:30px" class="box"><h2>Sana uygun görevler</h2><p class="muted">Gerçek doğrulanmış şirket görevleri sisteme eklendikçe burada görünecek. Demo görevler hiçbir zaman doğrulanmış deneyim gibi gösterilmez.</p></div></div></main>`;return `<header class="top"><div class="wrap topin">${brand()} <span>Şirket</span><button class="btn alt" onclick="out()">Çıkış</button></div></header><main class="dash"><div class="wrap"><div class="ey">Şirket Paneli</div><h1>${q.trade_name||q.legal_name}</h1><p class="muted">${q.legal_name} · ${q.sector} · ${q.city}</p><div class="notice">Doğrulama durumu: <b>${q.status==='pending'?'Bekleniyor':q.status}</b></div><div class="grid" style="margin-top:20px"><div class="box"><small>Yayındaki görev</small><strong>0</strong></div><div class="box"><small>Aday teslimi</small><strong>0</strong></div><div class="box"><small>Talent Radar</small><strong style="font-size:20px">Henüz boş</strong></div></div></div></main>`}catch(e){out();return''}}
function out(){token='';role='';localStorage.removeItem('genc_token');localStorage.removeItem('genc_role');go('/')}async function render(){let x=p(),h=x==='/'?home():x==='/student/register'?sreg():x==='/company/register'?creg():x==='/student/login'?login('student'):x==='/company/login'?login('company'):x==='/dashboard'?await dashboard():home();A.innerHTML=h;scrollTo(0,0)}addEventListener('hashchange',render);addEventListener('DOMContentLoaded',render)
</script></body></html>'''

@app.get('/',response_class=HTMLResponse)
def root(): return HTML
@app.get('/{path:path}',response_class=HTMLResponse)
def spa(path:str): return HTML
